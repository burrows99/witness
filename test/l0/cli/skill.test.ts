...
import { renderSkill, SPEC_FRONTMATTER_FIELDS, type SkillFacts } from '../../../src/cli/skill.js'
...
describe('what is derived from this project', () => {
  ...
  it('names the languages this project gates and the adapter each needs', () => {
    // Not whether they are installed. That is a fact about a machine, and
    // stating it here made `skill --check` compare environments rather than
    // projects — it reported "stale" on every CI run of every commit.
    expect(skill).toMatch(/\*\*ts\*\* — js-debug/)
    expect(skill).toMatch(/\*\*py\*\* — debugpy/)
    expect(skill).toMatch(/witness doctor/)
  })
...
describe('a project that has not adopted witness yet', () => {
  ...
  it('says nothing about what is installed, in either direction', () => {
    // The property that makes `--check` answerable: two machines with
    // different adapters must generate the same file.
    const here = renderSkill(facts())
    const elsewhere = renderSkill(facts({
      adapters: facts().adapters.map((a) => ({ ...a, available: !a.available })),
      browser: !facts().browser,
    }))
    expect(elsewhere).toBe(here)
  })

  it('says the web driver needs Playwright, without claiming to know if it is here', () => {
    // "Playwright is not installed" is true of a machine, not of a project,
    // and putting it here is what made two machines generate different files.
    expect(renderSkill(fresh())).toMatch(/needs Playwright/)
    expect(renderSkill(fresh())).toMatch(/doctor/)
  })
})
