...
| `SV011` | blocks | a probe was accepted but never verified — the line was never watched | Run `witness doctor` — this is a path-mapping problem, not a coverage one. |
...
- A bypass needs a reason and is published: apply the `witness:bypass` label and put
  `witness:bypass: <reason>` in the change description, or pass `--bypass "<reason>"` locally.
