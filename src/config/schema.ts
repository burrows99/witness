...
export type DatabaseConfig = {
  ...
  database: string;
  /**
   * Where the credential comes from — a secret source like any other, or a bare string.
   *
   * It was `password`, and a bare string only, which made this the one credential a description
   * FORCED into the file. "The config has nowhere else to put it" is the wrong reason for a
   * credential to be in a repository.
   *
   * Named for the source rather than the value, because that is what it now is — and because a field
   * called `password` in a type declaration is a thing every secret scanner is right to ask about,
   * even when, as here, there has never been a value anywhere near it. `password` still works.
   */
