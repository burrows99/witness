...
test("a service's secrets are scoped, so two services may each have a `password`", () => {
  const config = of({
    grafana: { secrets: { password: { env: "GRAFANA_DB_CREDENTIAL_SOURCE" } } },
    billing: { secrets: { password: { env: "BILLING_DB_CREDENTIAL_SOURCE" } } },
  });
...
test("a shared secret and a service's own can both exist", () => {
  const config = of({ grafana: { secrets: { password: { env: "GRAFANA_CREDENTIAL_SOURCE" } } } }, { secrets: { ciToken: { env: "SHARED_CI_CREDENTIAL_SOURCE" } } });
