...
export class Skill {
  ...
  private readonly product?: { apps: string[]; actions: { name: string; summary?: string }[]; queries: string[]; operations: string[] };
  ...
  render(): string {
    ...
    return [
      ...
      `# ${tool}`,
      "",
      "Generated by `witness skill` from the tool that runs — every list below is asked for at the",
      "moment it is printed, so it describes this version and not a remembered one. Regenerate after an",
      "upgrade rather than editing it.",
      "",
      ...this.intuition(),
      ...this.loop(),
      ...this.commandSection(),
  ...
  private intuition(): string[] {
    return [
      ...
      "That is a program, so it lives in a file — `.witness/specs/*.spec.ts` — and it produces the video,",
      "the frames and the note a reviewer reads.",
      "",
  ...
  private commandSection(): string[] {
    ...
    for (const command of this.commands) {
      lines.push(`- \`witness ${command.noun}\` — ${command.summary}`);
  ...
  private productSection(): string[] {
    ...
    lines.push("", "`witness action show <name>` prints the steps of any of them, as declared.", "");
    return lines;
  }
  ...
  private configSection(): string[] {
    ...
    lines.push("", "`witness config template` prints every field with its full documentation.", "");
