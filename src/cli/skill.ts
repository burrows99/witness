import { canonicalJson, FINDING_CATALOG, sha256, type GateCode } from '../core/index.js'
...
const REMEDIES: Partial<Record<GateCode, string>> = {
  ...
  SV011: 'Run `witness doctor` — this is a path-mapping problem, not a coverage one.',
...
metadata:
  ...
  witness-project: ${yamlString(facts.project)}
---

# Verifying changes in ${facts.project}

Generated by \`witness skill\` from this project's config, committed plans and installed
...
| \`4\` | **harness failure** | witness could not observe. Not your change's fault; run \`witness doctor\` and report it |
...
witness run --plan <plan-id> --record --json
...
/** What the tooling cannot do — the same on every machine, by design. */
function renderLimits(): string {
  ...
  // Nothing here about what is installed. Those are limits of a machine, and
  // `doctor` reports them; a limit of the *tool* is what belongs in a file
  // that has to read the same on every machine.
  lines.push('- A language whose adapter is absent is ungated (`SV016`), never silently passed —')
  lines.push('  run `witness doctor` to see which are absent here.')
  return lines.join('\n')
}

function renderAdapters(facts: SkillFacts): string {
  const lines: string[] = []
  const named = new Map(facts.adapters.map((a) => [a.language, a.name]))
  const gated = facts.scope.languages

  // Deliberately not "instrumentable *here*". Availability is a property of a
  // machine, and stating it in a file that describes a project made
  // `skill --check` unanswerable — it compared a file generated on a laptop
  // holding delve and debugpy against a runner holding neither, so it said
  // "stale" on every commit. `doctor` owns the machine; this owns the project.
  if (gated.length > 0) {
    lines.push('This project gates these languages, each through the adapter named:')
    lines.push('')
    for (const language of gated) {
      lines.push(`- **${language}** — ${named.get(language) ?? 'no adapter in this build'}`)
    }
    lines.push('')
    lines.push('Run `witness doctor` for whether *this* machine has them. A language whose adapter is')
    lines.push('missing reports ungated (`SV016`) rather than passing silently, so a run says what it')
    lines.push('could not watch instead of implying it watched everything.')
  } else {
    lines.push('This project gates no languages, so nothing can be held to line coverage. Set')
    lines.push('`scope.languages` in config to change that.')
  }

  lines.push('')
  lines.push('Drivers: `api` (HTTP) and `web` (needs Playwright — `doctor` says whether it is here).')
...
function fingerprintOf(facts: SkillFacts): string {
  return sha256(canonicalJson({
    ...
    // Names only. Whether an adapter is installed, and whether Playwright is
    // here, are facts about a *machine* — and including them made `--check`
    // compare environments instead of projects, so it reported "stale" on
    // every CI run of every commit, for ever.
    adapters: [...facts.adapters]
      .sort((a, b) => a.language.localeCompare(b.language))
      .map((a) => ({ language: a.language, name: a.name })),
...
function skillDescription(facts: SkillFacts): string {
  ...
  const description =
    ...
    `Use whenever you edit code here, before saying a change is done, or when a witness gate reports ` +
