...
test("a database password is a secret source, not a string the config has to hold", () => {
  ...
  const system = new System({
    ...
    database: { service: "postgres", user: "u", database: "d", password: { env: "PGPASSWORD_FOR_TEST" } },
  } as never);
  process.env.PGPASSWORD_FOR_TEST = "from the environment";
  ok(system.db, "the database is built without the password being written down");
  delete process.env.PGPASSWORD_FOR_TEST;
});
